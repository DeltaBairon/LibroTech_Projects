# 🚀 Proyecto Integrado Full Stack - Arquitectura Multi-Cloud
...
1. **Frontend** (Docker en EC2) → Solicita datos al Backend
2. **Backend** (Docker en EC2) → Recibe petición y consulta la API
3. **API REST** → Realiza query a Azure SQL Database
4. **Azure Firewall** → Valida IP de EC2 (whitelisted)
5. **Azure SQL** → Procesa query y retorna datos
6. **Backend** → Envía respuesta al Frontend
7. **Frontend** → Renderiza datos al usuario
---
